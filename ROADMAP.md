...
### v2.1 — Option C design system (best-effort) (July 2026)
- **PTO-601 · Design tokens.** Repointed `:root` (light + dark) to the Option C palette: **purple** primary (`#7C3AED` / dark `#A78BFA`) replacing blue, a **purple→magenta gradient** (`--gradient-primary`) with a matching glow (`--gradient-glow`), a **data-color scale** (`--data-cyan/purple/magenta/blue/teal` each with a `-soft` companion), **motion tokens** (`--dur-fast/--dur/--dur-slow/--ease`), and **purple focus rings** (`--ring`). Chart line/usage tokens moved to cyan (solid) + magenta (dashed) to match the locked mockup.
- **PTO-603 · Topbar / theme.** Theme control is now a **3-option segmented pill — light / dark / system** — with true OS-preference following (`system` resolves live via `matchMedia`, persisted as a mode in `pto_theme`). The **notification bell** shows its red dot only when there are unread insights (`notificationsSeen` state); clicking it marks them seen and jumps to Insights.
- **PTO-604 · KPI cards.** Each card leads with a **32×32 soft-color icon chip** (used→purple, sick→teal, personal holiday→magenta, anniversary→blue), values are **tinted to their data color**, and Used YTD gained a **cumulative-usage sparkline**. Vacation Balance keeps its semantic usage ring.
- **Brand + avatars** now use the gradient; added a **`prefers-reduced-motion`** guard. Migration-safe (`themeMode`/`notificationsSeen` defaulted in `load()` and import). Verified in Chromium across desktop/tablet/mobile/320px and light/dark/system — no overflow, no console errors; gradient CTA/brand, data chips, sparkline, 3-way theme, and notification badge all confirmed. Exact hex/spacing to be reconciled when the clean `DESIGN_v2.md` spec is available.
