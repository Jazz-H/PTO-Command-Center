...
import { getAllotment, countsTowardUsage } from "./balance.ts";
...
export function buildChartData(){
  ...
  for (let i = 0; i < 15; i++){
    ...
    const usedThisMonth = state.entries.filter(e => e.type === "PTO" && countsTowardUsage(e.status) && parseDate(e.date) >= monthStart && parseDate(e.date) <= monthEnd).reduce((s,e) => s + Number(e.hours||0), 0);
    const usedToDate = state.entries.filter(e => e.type === "PTO" && countsTowardUsage(e.status) && parseDate(e.date).getFullYear() === yr && parseDate(e.date) <= monthEnd).reduce((s,e) => s + Number(e.hours||0), 0);
...
export function usageThisVsLast(){
  ...
  const inMonth = (yy,mm) => state.entries.filter(e => e.type==="PTO" && countsTowardUsage(e.status) && parseDate(e.date).getFullYear()===yy && parseDate(e.date).getMonth()===mm).reduce((s,e)=>s+Number(e.hours||0),0);
...
export function vacCumulativeByMonth(year){
  ...
  for (let mo=0; mo<=upto; mo++){
    run += state.entries.filter(e => e.type==="PTO" && countsTowardUsage(e.status) && parseDate(e.date).getFullYear()===year && parseDate(e.date).getMonth()===mo).reduce((s,e)=>s+Number(e.hours||0),0);
    out.push(run);
  }
