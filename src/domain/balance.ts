...
export function ytdUsage(type: EntryType, year: number): number { return state.entries.filter(e => e.type===type && parseDate(e.date).getFullYear()===year).reduce((s,e) => s+Number(e.hours||0), 0); }
